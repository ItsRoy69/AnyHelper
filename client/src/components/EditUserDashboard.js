...
const EditUserDashboard = (props) => {
        ...
        uploadBytes(storageRef, profileImg).then((snapshot) => {
          if (snapshot) {
            ...
            getDownloadURL(ref(storage, `profileImages/${profileImg.name}`)).then(
              (url) => {
                Axios.patch(`https://anyhelper-server.onrender.com/customers/updateUser/${user._id}`, {
                  profileImg: url,
                })
                  .then((res) => {
                    console.log(res);
    ...
    const submitChanges = async (e) => {
        e.preventDefault();
        await Axios.patch(`https://anyhelper-server.onrender.com/customers/updateUser/${user._id}`,editUser)
        .then((res) => {
            console.log(res);
