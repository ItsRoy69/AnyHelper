...
const UserDashboard = () => {
  ...
  const fetchUser = async () => {
    if (type == 0) {
      await axios
        .get(`https://anyhelper-server.onrender.com/stores/get-user/${user._id}`)
        .then((response) => {
    ...
    if (type == 1) {
      await axios
        .get(`https://anyhelper-server.onrender.com/workers/get-user/${user._id}`)
        .then((response) => {
          setLoggedInUser(response.data);
          setItems(response.data.items);
        })
    ...
    if (type == 2) {
      await axios
        .get(`https://anyhelper-server.onrender.com/customers/get-user/${user._id}`)
        .then((response) => {
  ...
  const deleteItem = async (item) => {
    await axios
      .post("https://anyhelper-server.onrender.com/stores/delete-item", {
        email: user.email,
        item: item,
      })
      .then((response) => {
